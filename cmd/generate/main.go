...
func main() {
	...
	for _, bc := range cfg.Branches {
		...
		for _, f := range files {
			...
			webContentLink := ""
			if drive.IsSupportedBook(f) {
				md = metadata.Extract(ctx, client, f)
				webContentLink, err = getWebContentLink(ctx, apiKey, f.ID)
				if err != nil {
					fatal(fmt.Sprintf("webContentLink %s: %v", f.ID, err))
				}
			}
			converted = append(converted, FileEntry{
				ID: f.ID, ParentID: f.ParentID, Name: f.Name, MIME: f.MIMEType, Size: f.Size,
				Modified: f.ModifiedTime, Checksum: f.MD5Checksum, IsFolder: drive.IsFolder(f), Metadata: md,
...
// getWebContentLink asks Google Drive for the official browser download URL
// for a public binary file. The link is generated during the catalog build and
// stored in the generated OPDS acquisition link; X4 does not need Drive API
// credentials.
func getWebContentLink(ctx context.Context, apiKey, fileID string) (string, error) {
	q := url.Values{}
	q.Set("key", apiKey)
	q.Set("fields", "id,webContentLink")
	u := "https://www.googleapis.com/drive/v3/files/" + url.PathEscape(fileID) + "?" + q.Encode()
	...
	// Some Drive files may not expose webContentLink. In that case return an
	// empty link so acquisitionURL can safely fall back to the existing URL.
	return strings.TrimSpace(result.WebContentLink), nil
}
...
(function(){
  ...
  function renderContent(){
    ...
    rows.forEach(function(f){
      ...
      else {var authors=(f.metadata&&f.metadata.authors||[]).join(', ');var subtitle=authors||(f.metadata&&f.metadata.title)||displayDate(f.modified);tr.innerHTML='<td><img class="cover" loading="lazy" src="'+cover(f.id)+'" alt="" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'grid\'"/><span class="cover-fallback" style="display:none">'+esc(ext(f.name))+'</span></td><td class="name-cell"><strong>'+esc(f.name)+'</strong><small>'+esc(subtitle)+'</small></td><td>'+esc(authors)+'</td><td>'+esc(ext(f.name))+'</td><td>'+esc(size(f.size))+'</td><td>'+esc(displayDate(f.modified))+'</td>';tr.onclick=function(){state.selected=f.id;renderContent();showBook(f,b)};}
      tb.appendChild(tr);
    });
  ...
  function showBook(f,b){
    var m=f.metadata||{};var authors=(m.authors||[]).join(', ');var body='<div class="detail"><dt>Tên file</dt><dd>'+esc(f.name)+'</dd><dt>Tên sách</dt><dd>'+esc(m.title||'—')+'</dd><dt>Tác giả</dt><dd>'+esc(authors||'—')+'</dd><dt>Nhà xuất bản</dt><dd>'+esc(m.publisher||'—')+'</dd><dt>Năm</dt><dd>'+esc(m.year||'—')+'</dd><dt>ISBN</dt><dd>'+esc(m.isbn||'—')+'</dd><dt>Ngôn ngữ</dt><dd>'+esc(m.language||'—')+'</dd><dt>Bộ sách</dt><dd>'+esc(m.series||'—')+'</dd><dt>Mô tả</dt><dd>'+esc(m.description||'—')+'</dd><dt>Định dạng</dt><dd>'+esc(ext(f.name))+'</dd><dt>Dung lượng</dt><dd>'+esc(size(f.size))+'</dd><dt>Cập nhật</dt><dd>'+esc(displayDate(f.modified))+'</dd><dt>MIME</dt><dd>'+esc(f.mime||'')+'</dd><dt>Checksum</dt><dd>'+esc(f.checksum||'—')+'</dd></div><div class="modal-actions"><button id="downloadBtn">Truy cập / tải</button><button id="opdsBtn">Mở OPDS</button></div>';
...
func writeCatalogJSON(out string, branches []Branch) error {
	...
	for _, br := range branches {
		...
		for _, f := range br.Files {
			jb.Files = append(jb.Files, JFile{f.ID, f.ParentID, f.Name, f.MIME, f.Size, f.Modified, f.Checksum, f.IsFolder, f.Metadata})
		}
		rows = append(rows, jb)
	}
...
func xmlHead(title string) string {
	return `<?xml version="1.0" encoding="UTF-8"?><feed xmlns="http://www.w3.org/2005/Atom" xmlns:opds="http://opds-spec.org/2010/catalog" xmlns:dc="http://purl.org/dc/elements/1.1/"><title>` + esc(title) + `</title>`
}
...
func writeFolder(dir, base string, br Branch, folderID string, root bool) error {
	...
	for _, f := range rows {
		if f.IsFolder {
			...
			modified := f.Modified
			x.WriteString(`<entry><title>` + esc(firstNonEmpty(f.Metadata.Title, f.Name)) + `</title>`)
			x.WriteString(`<updated>` + esc(modified) + `</updated>`)
			if len(f.Metadata.Authors) > 0 {
				x.WriteString(`<author><name>` + esc(strings.Join(f.Metadata.Authors, ", ")) + `</name></author>`)
			}
			if f.Metadata.Publisher != "" {
				x.WriteString(`<dc:publisher>` + esc(f.Metadata.Publisher) + `</dc:publisher>`)
			}
			if f.Metadata.Language != "" {
				x.WriteString(`<dc:language>` + esc(f.Metadata.Language) + `</dc:language>`)
			}
			if f.Metadata.Description != "" {
				x.WriteString(`<summary type="text">` + esc(f.Metadata.Description) + `</summary>`)
			}
