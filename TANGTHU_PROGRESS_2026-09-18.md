...
# 23. Tách Metadata Job thành repository riêng — ý tưởng kiến trúc mới

### Lý do

Hai loại công việc có thời gian chạy rất khác nhau:
...
Không nên để metadata extraction trở thành điều kiện để một tủ sách xuất hiện.

### Kiến trúc đề xuất

Giữ `tangthu-opds` làm repository chính cho catalog:

```text
Repo chính: tangthu-opds
│
├── config/branches.json
├── generator
├── catalog
...
Tạo một repository riêng cho metadata:

```text
Repo phụ: tangthu-metadata
│
├── metadata/
│   └── <Google-Drive-file-ID>.json
├── covers/
│   └── <Google-Drive-file-ID>.<ext>
└── manifest.json
```
...
```text
          ...
          │                         │
          ▼                         ▼
     CATALOG JOB              METADATA JOB
     Repo chính              Repo riêng
          │                         │
          │                    EPUB extract
          │                    metadata/cover
          │                         │
          ▼                         ▼
    catalog nhanh             metadata cache
          │                         │
          └────────────┬────────────┘
                       ▼
                 GitHub Pages
...
1. **Catalog job không chờ metadata job.**
2. Tạo/sửa/xóa tủ không bị chặn bởi metadata đang chạy.
3. Metadata extraction chạy độc lập và có thể xử lý lâu.
4. Metadata được lưu theo Google Drive file ID để có thể cache và tái sử dụng.
5. Nếu metadata chưa có hoặc extraction thất bại, file vẫn xuất hiện trong catalog.
6. Metadata đã có thì không cần extract lại nếu dữ liệu nguồn chưa thay đổi.
7. Không tạo một PR cho từng cuốn sách; ưu tiên metadata repo tự commit theo batch/job.
8. Phải thiết kế cơ chế publish/merge để metadata repo không làm hỏng catalog repo.
9. Đây là **kiến trúc đề xuất**, chưa triển khai và chưa thay thế MVP hiện tại.
...
Tủ A tạo lúc 20:00
    ↓
catalog READY rất nhanh
    ↓
metadata A chạy nền

Tủ B tạo lúc 20:01
    ↓
catalog B vẫn có thể READY
    ↓
không phải chờ metadata A
```
...
### Các điểm phải nghiên cứu trước khi triển khai

- Cách trigger metadata job từ catalog repo.
- Một job xử lý một file hay một batch.
- Cách metadata repo báo trạng thái về repo chính.
- Cách catalog lấy metadata về khi build.
- Cách tránh race condition khi nhiều Actions cùng chạy.
- Cách version/cache metadata theo `fileId`, modified time hoặc checksum.
- Cách xử lý cover và metadata khi file Google Drive bị thay thế.
- Có cần metadata repo public hay chỉ dùng làm build source.
- Giới hạn GitHub Actions, repository size và số lượng commit.
- Có nên dùng artifact/release thay vì commit trực tiếp metadata.
- Có cần một publish job cuối cùng hay catalog có thể đọc metadata repo trực tiếp.

**Chưa chọn implementation cụ thể.**
...
# 24. Những gì đã biết về Apps Script
...
# 25. Điểm bảo mật quan trọng của Apps Script
...
# 27. Câu hỏi kiến trúc cần nghiên cứu tiếp
...
# 28. Nguyên tắc không nên phá vỡ
...
- [ ] thiết kế metadata repo riêng
- [ ] metadata job độc lập với catalog job
...
**Ưu tiên 2:** thiết kế chi tiết Catalog Job và Metadata Job độc lập.

**Ưu tiên 3:** nghiên cứu repository metadata riêng: schema, trigger, queue, cache, publish và concurrency.

**Ưu tiên 4:** tiếp tục đào sâu Google Apps Script sau khi có thiết kế metadata repo.

**Chưa triển khai metadata repo cho tới khi chốt được cơ chế đồng bộ và publish.**
